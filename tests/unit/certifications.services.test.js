...
describe('Testing getCertifications() funtionalities', () => {
    ...
    test('Should return all records of certificates when function is called', async () => {
        ...
        const mock_certificates = [
            ...
            {first_name: 'Bruce', last_name: 'Wayne', id: 1, member_id: 1, created_at: '2025-07-23', crn: 1},
            {first_name: 'Clark', last_name: 'Man', id: 2, member_id: 2, created_at: '2025-04-13', crn: 2},
            {first_name: 'Spider', last_name: 'Man', id: 3, member_id: 3, created_at: '2024-01-20', crn: 3}
        ];
        ...
        expect(mockDB.query).toHaveBeenCalledWith('SELECT m.first_name, m.last_name, c.* FROM certifications c JOIN members m ON c.member_id = m.id');

        expect(result).toEqual([

            {first_name: 'Bruce', last_name: 'Wayne', id: 1, member_id: 1, created_at: '2025-07-23', crn: '0001'},
            {first_name: 'Clark', last_name: 'Man', id: 2, member_id: 2, created_at: '2025-04-13', crn: '0002'},
            {first_name: 'Spider', last_name: 'Man', id: 3, member_id: 3, created_at: '2024-01-20', crn: '0003'}
        ]);
    })
...
describe('Testing getCertificationById() functionalities', () => {
    ...
    test('Returns the Certificate record based on ID if it exist', async() => {
        ...
        //mock database
        const mock_certificate = {first_name: 'Bruce', last_name: 'Wayne', id: 1, member_id: 1, created_at: '2025-07-23', crn: 1}
        ...
        //expect actual function logic to be correct
        expect(mockDB.query).toHaveBeenCalledWith('SELECT * FROM certifications WHERE id = ?', [1]);
    ...
    test('Returns nothing if Certificate record is not found based on ID', async() => {
        ...
        //expect actual function logic to be correct
        expect(mockDB.query).toHaveBeenCalledWith('SELECT * FROM certifications WHERE id = ?', [2])
...
describe('Testing createCertification() functionalities', async() => {
    ...
    test('Create certificate record based on member_id', async() => {
        ...
        expect(mockDB.query).toBeCalledWith('SELECT MAX(crn) AS curr_crn FROM certifications');
        expect(mockDB.execute).toBeCalledWith('INSERT INTO kabuhayan_db.certifications (`member_id`, `created_at`, `crn`) VALUES (?, ?, ?)', [1, fakeDate, 2])
    ...
    test('Create certificate record for the first time', async() => {
        ...
        expect(mockDB.query).toBeCalledWith('SELECT MAX(crn) AS curr_crn FROM certifications');
        expect(mockDB.execute).toBeCalledWith('INSERT INTO kabuhayan_db.certifications (`member_id`, `created_at`, `crn`) VALUES (?, ?, ?)', [1, fakeDate, 1])
...
describe('Testing updateCertification() functionalities', () => {
    ...
    test('Update Certificate record`s column and return the affected row if you try to update only 1 column', async() => {
        ...
        //run actual function
        const result = await CertificationsServices.updateCertification(id, updates);

        //expect actual function logic to be correct
        expect(mockDB.execute).toHaveBeenCalledWith('UPDATE kabuhayan_db.certifications SET `member_id` = ? WHERE id = ?', [1, 2])
    ...
    test('Throws error if you try to update 2 or more columns', async() => {
        ...
        await expect(CertificationsServices.updateCertification(id, updates)).rejects.toThrow('Only one valid column can be updated at a time.')
    });
...
describe('Testing updateCertificationMultiple() functionalities', () => {
    ...
    const result = await CertificationsServices.updateCertificationMultiple(id, updates)

    //Expect function to run properly
    expect(mockDB.execute).toHaveBeenCalledWith('UPDATE kabuhayan_db.certifications SET `member_id` = ?, `created_at` = ? WHERE id = ?', [1, '2020-01-02', 3])
    expect(result).toEqual({affectedRows: 1})
    })
    ...
    await expect(CertificationsServices.updateCertificationMultiple(id, updates)).rejects.toThrow(`Attempted to update an unauthorized column: Loan_Sharker`);
    })
    ...
    //Run the function
    await expect(CertificationsServices.updateCertificationMultiple(id, updates)).rejects.toThrow(`No valid columns provided for update.`);
...
describe('Testing deleteCertification() functionalities', () => {
  ...
  test('Deletes successfully', async() => {
    ...
    expect(mockDB.execute).toBeCalledWith('DELETE FROM kabuhayan_db.certifications WHERE id = ?', [1])
  ...
  test('Deletes nothing if id is not found', async() => {
    ...
    expect(mockDB.execute).toBeCalledWith('DELETE FROM kabuhayan_db.certifications WHERE id = ?', [100])
