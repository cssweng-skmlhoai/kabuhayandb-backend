    ...
    const mockFamilyMembers = [
      {
        ...
        first_name: 'Madly',
        middle_name: 'Abydos',
        birth_date: '2004-12-13',
        gender: 'M',
        relation_to_member: 'Father',
        member_id: 2,
...
describe('Testing createFamilyMembers() functionalities', () => {
  ...
  test('Inserts FamilyMember record in database and returns that record that was inserted WHILE using an existing connection', async () => {
    ...
    //run actual function
    const result = await FamilyMembersService.createFamilyMember(
      data,
      mockConn
    );
...
describe('Testing updateFamilyMember() functionalities', async () => {
  ...
  test('Update Family record`s column and return number of affectedRows if you try to update only 1 column', async () => {
    ...
    //expect actual function logic to be correct
    expect(mockDB.execute).toHaveBeenCalledWith(
      'UPDATE kabuhayan_db.family_members SET `relation_to_member` = ? WHERE id = ?',
      ['Daughter', 2]
    );
  ...
  test('Updates birth_date column when specified in updates and returns number of affectedRows', async () => {
    ...
    expect(mockDB.execute).toHaveBeenCalledWith(
      'UPDATE kabuhayan_db.family_members SET `birth_date` = ? WHERE id = ?',
  ...
  test('Updates birth_date column when specified but with invalid date format', async () => {
    ...
    //Expect function to run properly
    await expect(
      FamilyMembersService.updateFamilyMember(id, updates)
    ).rejects.toThrow(`Invalid date format for birth_date: invalid`);
  });
...
describe('Testing updateFamilyMemberMultiple() functionalities', async () => {
  ...
  test('Updates multiple columns and returns the number of affected rows', async () => {
    ...
    const result = await FamilyMembersService.updateFamilyMemberMultiple(
      id,
      updates
    );
    ...
    //Expect function to run properly
    expect(mockDB.execute).toHaveBeenCalledWith(
      'UPDATE kabuhayan_db.family_members SET `relation_to_member` = ?, `educational_attainment` = ? WHERE id = ?',
      ['Daughter', 'College', 3]
    );
    expect(result).toEqual({ affectedRows: 1 });
  });
  ...
  test('Updates birth_date column when specified in updates and returns number of affectedRows', async () => {
    ...
    //Expect function to run properly
    expect(mockDB.execute).toHaveBeenCalledWith(
      'UPDATE kabuhayan_db.family_members SET `relation_to_member` = ?, `educational_attainment` = ?, `birth_date` = ? WHERE id = ?',
      ['Daughter', 'College', new Date('2008-06-07'), 3]
    );
  ...
  test('Updates birth_date column when specified but with invalid date format', async () => {
    ...
    //run actual function //expect actual function logic to be correct
    await expect(
      FamilyMembersService.updateFamilyMemberMultiple(id, updates)
    ).rejects.toThrow(`Invalid date format for birth_date: invalid-date`);
  });
